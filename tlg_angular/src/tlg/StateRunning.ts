...
export class StateRunning extends State {
    ...
    private initPreview(gc:PlatformContext) {
        var shape=this.random(this.context.currentScore.getLevel() * StateRunning.SHAPE_PER_LEVEL)+1;
        var color=gc.getColor(shape%gc.countOfColor());
        var turn=this.random(4);
    ...
    private gameOverState(p:PlatformContext):StateID {
        var highscoreList = new HighscoreList(p);
