...
export class MatrixVisible extends Matrix {
    ...
    public setPixelGeometry(rect:TlgRectangle):void {
        ...
        // center
        var xoffset:number = Math.floor(rect.getL() + (rect.getWidth()  - (squareSize * this.getWidth()) ) / 2);  
        var yoffset:number = Math.floor(rect.getT() + (rect.getHeight() - (squareSize * this.getHeight()) ) / 2);  
    ...
    private getPixelArea(rect:TlgRectangle):TlgRectangle {
        let ret= new TlgRectangle(0,0,0,0);
        
        if (rect.getWidth() > 0 && rect.getHeight() > 0) {
            let tl=this.getXY(rect.getL(),rect.getT());
            let br=this.getXY(rect.getR(), rect.getB());
    ...
    public updateAll(gc:PlatformContext):void {
        var r:TlgRectangle=TlgRectangle.copy(this.getPixelGeometry());
        r.grow(1);
