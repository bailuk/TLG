...
export class MatrixWithShape extends MatrixManipulator {
    ...
    public moveShapeLeft () {
        const pos = this.movingShape.getPos()
        pos.setX(pos.getX() - 1)
    ...
    public moveShapeTurn () {
        const temp = this.movingShape.getRotatedCopy(1)
    ...
    public centerMovingShape () {
        const r = this.movingShape.getActiveArea()
        const x = ((this.getWidth() - r.getWidth()) / 2) - r.getL()
        const y = ((this.getHeight() - r.getHeight()) / 2) - r.getT()
