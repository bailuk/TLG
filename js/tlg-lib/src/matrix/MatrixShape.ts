...
export class MatrixShape extends Matrix {
    ...
    private copyAndRotateMatrix (source:MatrixShape, direction:number) {
        ...
        if (direction === 1) {
            for (let x = 0; x < size; x++) {
                for (let y = 0; y < size; y++) { this.getXY(x, y).set(source.getXY(y, max - x)) }
            }
        } else if (direction === 2) {
            for (let x = 0; x < size; x++) {
                for (let y = 0; y < size; y++) { this.getXY(x, y).set(source.getXY(max - x, max - y)) }
            }
        } else {
            for (let x = 0; x < size; x++) {
                for (let y = 0; y < size; y++) { this.getXY(x, y).set(source.getXY(max - y, x)) }
            }
        }
    }

    private adjustOffset (source:MatrixShape) {
        const sourceRect:TlgRectangle = source.getActiveArea()
        const destRect:TlgRectangle = this.getActiveArea()
    ...
    public setColor (c:number) {
        for (let i = 0; i < this.count(); i++) { this.get(i).setColor(c) }
    }

    public getActiveArea () {
        const r:TlgRectangle = new TlgRectangle(this.getWidth() - 1, this.getHeight() - 1, 0, 0)
    ...
    public autoOffset () {
        const r:TlgRectangle = this.getActiveArea()
