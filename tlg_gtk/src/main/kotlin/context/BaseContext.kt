...
open class BaseContext : PlatformContext() {
    ...
    companion object {
        ...
        private const val COLOR_GRID = PALETTE_SIZE - PALETTE_RESERVED
        ...
        private val palette = ArrayList<RGBA>().apply {
            ...
            for (i in 0 until PALETTE_SIZE - PALETTE_RESERVED) {
                val rgb = ColorHelper.convertHSVtoRGB(h, 1f, 1f)
                add(createRGBA(rgb[0], rgb[1], rgb[2]))
                h++
                h %= 6f
            }
            val x = 1f / 256f
            add(createRGBA(x*178, x*178, x*178)) // Grid
            add(createRGBA(x * 44, x * 67, x * 77, 0f)) // Background
            add(createRGBA(x * 218, x * 234, x * 223)) // Highlight
            add(createRGBA(x * 10, x * 10, x * 10)) // Dark
            add(createRGBA(0f, 0f, 0f))  // Frame
            add(createRGBA(x* 136, x* 136, x* 136)) // Grayed
        }

        private fun createRGBA(r: Float, g: Float, b: Float, a: Float = 1f): RGBA {
            return RGBA().apply {
                fieldRed = r
                fieldGreen = g
                fieldBlue = b
