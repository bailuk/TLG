...
public class Actions {
    ...
    public void setAccels(String name, String[] accels) {
        if (accels != null && accels.length > 1 && accels[accels.length - 1] == null) {
            var detailedActionName = new Str(this.name + "." + name);
            var strsAccels = new Strs(accels);
            app.setAccelsForAction(detailedActionName, strsAccels);
            strsAccels.destroy();
            detailedActionName.destroy();
        }
    }
}
